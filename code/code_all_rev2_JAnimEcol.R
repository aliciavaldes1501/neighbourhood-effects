...
#based on a connectivity matrix of pairwise Euclidean distances among the shoots up to a distance of 5 m. 
data_plants_paper.nb1_short <- dnearneigh(subset(data_plants_paper,!is.na(phen)), 0, 5) #Create neighbours matrix (5 m)
data_plants_paper.listw1_short <- nb2listw(data_plants_paper.nb1_short,zero.policy=T)

moran_model1_short<- moran.test(res_model1, listw=data_plants_paper.listw1_short,zero.policy=T) 
moran_model1_short #Significant autocorrelation in the residuals of model1
...
MEa.model1 <-ME(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3)+scale(phen_n3) + scale(phen):scale(Mrub_sum)+
                  scale(phen):scale(phen_n3) + scale(pldens_3):scale(phen_n3),listw=data_plants_paper.listw1,
                data=subset(data_plants_paper,!is.na(phen)),family=binomial,alpha=0.05,stdev=T,verbose=T) 
MEb.model1 <-ME(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3)+scale(phen_n3) + scale(phen):scale(Mrub_sum)+
                  scale(phen):scale(phen_n3) + scale(pldens_3):scale(phen_n3),listw=data_plants_paper.listw1_short,
                data=subset(data_plants_paper,!is.na(phen)),family=binomial,alpha=0.05,stdev=T,verbose=T) 
...
model1_MEb<-glm(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3) +
                  scale(phen_n3) + scale(phen):scale(Mrub_sum)+
                  scale(phen):scale(phen_n3) + scale(pldens_3):scale(phen_n3)+
                  scale(vector3)+scale(vector4)+scale(vector5),
                subset(data_plants_paper,!is.na(phen)),family="binomial")
summary(model1_MEb) # Interaction NS
NagelkerkeR2(model1_MEb)

vector6<-MEb.model1$vectors[,1] #with stdev=T, dist=5m
vector7<-MEb.model1$vectors[,2]
vector8<-MEb.model1$vectors[,3]
vector9<-MEb.model1$vectors[,4]
vector10<-MEb.model1$vectors[,5]
vector11<-MEb.model1$vectors[,6]
vector12<-MEb.model1$vectors[,7]
vector13<-MEb.model1$vectors[,8]
vector14<-MEb.model1$vectors[,9]

model1_MEc<-glm(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3) +
                  scale(phen_n3) + scale(phen):scale(Mrub_sum)+
                  scale(phen):scale(phen_n3) + scale(pldens_3):scale(phen_n3)+
                  scale(vector6)+scale(vector7)+scale(vector8)+scale(vector9)+scale(vector10)+
                  scale(vector11)+scale(vector12)+scale(vector13)+scale(vector14),
                subset(data_plants_paper,!is.na(phen)),family="binomial")
summary(model1_MEc) # Interaction NS
NagelkerkeR2(model1_MEc)
...
moran_model1_ME #Significant autocorrelation in the residuals of model1_ME
...
data_model1a<-subset(data_plants_paper,!is.na(phen))
model1a<-glm(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3) +
               scale(phen_n3) + scale(phen):scale(Mrub_sum)+
               scale(phen):scale(phen_n3) + scale(pldens_3):scale(phen_n3),
             data_model1a,family="binomial",na.action="na.fail") 
step(model1a) # Suggests removing interaction scale(phen):scale(phen_n3)

dredge(model1a) # Only first model with delta < 2 --> Suggests removing interaction scale(phen):scale(phen_n3)

model1b<-glm(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3) +
               scale(phen_n3) + scale(phen):scale(Mrub_sum)+
               scale(pldens_3):scale(phen_n3),
             data_model1a,family="binomial",na.action="na.fail")  # New model with that interaction removed
summary(model1b)
NagelkerkeR2(model1b)
AIC(model1a)
AIC(model1b)

res_model1b<-residuals(model1b) #Get residuals of model

moran_model1b<- moran.test(res_model1b, listw=data_plants_paper.listw1) 
moran_model1b #Significant autocorrelation in the residuals of model1b

ME.model1b <-ME(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3)+scale(phen_n3) + scale(phen):scale(Mrub_sum)+
                  scale(pldens_3):scale(phen_n3),listw=data_plants_paper.listw1,
                data=subset(data_plants_paper,!is.na(phen)),family=binomial,alpha=0.05,stdev=T,verbose=T) 

vector15<-ME.model1b$vectors[,1]
vector16<-ME.model1b$vectors[,2]

model1b_ME<-glm(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3) +
                  scale(phen_n3) + scale(phen):scale(Mrub_sum)+
                  scale(pldens_3):scale(phen_n3)+scale(vector15)+scale(vector16),
                data_model1a,family="binomial",na.action="na.fail") # Repeat model with eigenvectors as predictors
summary(model1b_ME) #Interaction p=0.05

res_model1b_ME<-residuals(model1b_ME) #Get residuals of model

moran_model1b_ME<- moran.test(res_model1b_ME, listw=data_plants_paper.listw1) 
moran_model1b_ME #Significant autocorrelation in the residuals of model2

# ME with dist matrix 5 m
ME.model1b_short <-ME(attack ~ scale(phen) + scale(Mrub_sum) + scale(pldens_3)+scale(phen_n3) + scale(phen):scale(Mrub_sum)+
                        scale(pldens_3):scale(phen_n3),listw=data_plants_paper.listw1_short,
                      data=subset(data_plants_paper,!is.na(phen)),family=binomial,alpha=0.05,stdev=T,verbose=T) 

vector17<-ME.model1b_short$vectors[,1]
# ...
...
#Checking for spatial autocorrelation in the residuals of model3_ME --> REPEAT
